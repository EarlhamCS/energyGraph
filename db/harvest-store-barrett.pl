...
$timestamp = time; 
$pReal = `java --cp /home/energy/scripts/production ModbusReadDemand 159.28.165.101 15.625`;
chomp($pReal); 
$pReal =~ s/"//g; 
# printf("At %d pReal = %d\n",  $timestamp, $pReal); 
...
$dbh->do("insert into electrical_energy values ('Barrett', $pReal, to_timestamp($timestamp))"); 
