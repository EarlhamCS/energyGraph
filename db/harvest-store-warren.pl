...
$timestamp = time; 
$pReal = `java --cp /home/energy/scripts/production ModbusReadDemand 159.28.165.103 62.5`;
chomp($pReal); 
$pReal =~ s/"//g; 
# printf("At %d pReal = %d\n",  $timestamp, $pReal); 
...
$dbh->do("insert into electrical_energy values ('Warren', $pReal, to_timestamp($timestamp))"); 
